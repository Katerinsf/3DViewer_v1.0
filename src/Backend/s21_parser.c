...
// Counts the number of vertices and surfaces;
// If v or f == 0, return an error
...
// Adds vertex coordinates and points on surfaces
...
// Adds vertex coordinates in one line to Data
int add_vertice(const char *str, obj_data *Data, int count_V) {
  ...
  str = strchr(str, ' ');
  while (error == OK && str &&
         i < 3) {  // Ignore everything at the end of the line
    str++;
...
// Counts the number of points in one line f
// checks if the first N>=3 points are correct, N <= num_V
...
// Adds points on a surface in one line to Data
// Error if point number > num_V
...
// Converts all points to a new coordinate system with a twisted center
